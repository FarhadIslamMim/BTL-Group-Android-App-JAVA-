...
public class CustomerFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        ((AppCompatActivity) context).setTitle("Customer List");
        if(getArguments() != null){
            //dealerId = getArguments().getInt("id");
            getCustomerList(getArguments().getInt("id"));
            Dealer dealer = r.where(Dealer.class).equalTo("id", getArguments().getInt("id")).findFirst();
            if (dealer != null){
                ((AppCompatActivity) context).setTitle("Dealer: "+dealer.getName());
            }
        } else {
        ...
        btnAddNew.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Fragment fragment = new CustomerCreateFragment();
                Bundle b = new Bundle();
                b.putInt("month", 1);
                b.putInt("year", 2022);
                fragment.setArguments(b);
                ((AppCompatActivity) context).getSupportFragmentManager().beginTransaction().replace(R.id.container, fragment).addToBackStack("add_customer").commit();
            }
        });
        return root;
    }
    ...
    public void getCustomerList(int id) {
        ...
        mCompositeDisposable = new CompositeDisposable();
        mCompositeDisposable.add(apiServices.getCustomersByDealer(token, id) //test jsonblob
                ...
                .subscribeWith(new DisposableObserver<ResponseDataList<Customer>>() {
                    ...
                    public void onError(Throwable e) {
                        loadingDialog.dismiss();
                        //ToastUtils.longToast("Server error");
                        ToastUtils.longToast("No customer found for this dealer");
                    }
                    @Override
                    public void onNext(ResponseDataList<Customer> value) {
                        if (value.isSuccess() && value.getDataList() != null) {
                            setCustomerList(value.getDataList());
                        } else {
                            ToastUtils.longToast("No customer found for this dealer");
                        }
                    }
    ...
    public void setCustomerList(List<Customer> customers) {
        ...
        if(customers == null){
            customers = r.where(Customer.class)
                    .findAll();
        }
